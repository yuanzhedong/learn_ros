...
class BlobDetector:
    ...
    def __init__(self, thr_min, thr_max, blur=15, blob_params=None, detection_window=None):
        ...
        self.image_sub = rospy.Subscriber("/camera/usb/0/image",Image,self.callback)
        print ("<< Subscribed to topic /camera/usb/0/image")
...
def main(args):
    ...
    co = np.uint8([[[0,255,255]]]) # yellow
    # define range of blue color in HSV
    hsv_val = cv2.cvtColor(co, cv2.COLOR_BGR2HSV)
    
    lower_hsv = hsv_val[0][0][0] - 10, 100, 100
    upper_hsv = hsv_val[0][0][0] + 10, 255, 255
    ...
    ic = BlobDetector(lower_hsv, upper_hsv, blur, params, detection_window)
